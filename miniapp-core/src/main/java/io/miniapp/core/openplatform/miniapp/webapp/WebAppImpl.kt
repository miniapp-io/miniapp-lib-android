...
internal class WebAppImpl(
    ...
    private var openWeb3Provider: WebBridgetProvider

   private var eventHandler: IWebAppEventHandler? = null

    init {
        telegramProvider = WebBridgetProvider(webView, TELEGRAM, ::postEvent)
        openWeb3Provider = WebBridgetProvider(webView, webAppName, ::postEvent)
    }
    ...
    private fun notifyEvent(event: String, eventData: JSONObject?) {
        telegramProvider.checkWebApp(false) {
            if (telegramProvider.isAppKnown()) {
                ...
                openWeb3Provider.notifyEvent(event, eventData)
            }
        }
    }
    ...
    internal class WebBridgetProvider(private val webView: WebView,
        ...
        @JavascriptInterface
        fun sayHello() {
            LogTimber.tag(TAG).d("Hello! Welcome to OpenWeb3 Android App. ")
            notifyEvent( "webview:notify", JSONObject().put("msg", "Welcome to Android OpenWeb3 MiniApp!" ))
        }
