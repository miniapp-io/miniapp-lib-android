...
import io.miniapp.core.openplatform.common.data.OpenWeb3Repository
import io.miniapp.core.openplatform.common.network.error.OpenWeb3Error
...
internal class MiniAppServiceImpl : MiniAppService {
    ...
    var miniAppHost = listOf("https://openweb3.io")
    var appName = "Sample"
    var webAppName = "OpenWeb3"
    ...
    fun parasError(throwable: Throwable): OpenWeb3Error {
        if (throwable is OpenWeb3Error) {
            appDelegate?.onApiError(throwable.code, throwable.error)
            return throwable
        }

        return (throwable as? HttpException)?.let {
            val openWeb3Error = it.response()?.toFailure() ?: OpenWeb3Error(it.code(), it.message())
            appDelegate?.onApiError(openWeb3Error.code, openWeb3Error.error)
            openWeb3Error
        }  ?: OpenWeb3Error(404, null)
    }
        ...
        val dApp: DAppDto? = config.id?.let {
            getDAppInfoById(it).let { result ->
                if (result.isSuccess()) {
                    ...
                    if (error is OpenWeb3Error && error.code == 460) {
        ...
        try {
            ...
            if (isWebAppShortLink(uri)) {
                owner.lifecycleScope.launch {
                    if(schemeRouter(url =url, callback = { app, dApp, params ->
                            dApp?.let { dapp->
                                WebAppParameters.Builder()
                                    .context(context)
                                    .owner(owner)
    ...
    private suspend fun schemeRouter(url: String,
                                      callback : (MiniAppDto?, DAppDto?, String?) -> Boolean): Boolean  {
        try {
            ...
            if (segments.size == 2) {
                if(segments[0].lowercase() == "apps") {
                    ...
                    return getMiniAppById(segments[1]).takeIf {
                        ...
                        callback((it as DataResult.Success).data, null, startParams)
                    } ?: false
                    ...
                    return getDAppInfoById(segments[1]).takeIf {
                        ...
                        val startParams = uri.getQueryParameter("startapp")
                        callback(null, (it as DataResult.Success).data, startParams)
                    } ?: false
                }
    ...
    fun openUrl(launchConfig: WebAppParameters, url: String, isInternal: Boolean) {
        if (isInternal) {
            launchConfig.owner.lifecycleScope.launch {
                if(schemeRouter(url =url, callback = { app, dapp, startParams ->
                        dapp?.let {
                            WebAppParameters.Builder()
