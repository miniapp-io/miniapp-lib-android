...
(function () {
  ...
  // MiniAppX apps will implement this logic to add service params (e.g. tgShareScoreUrl) to game URL
  ...
  window.MiniAppXGameProxy_receiveEvent = receiveEvent;

  // App backward compatibility
  window.MiniAppXGameProxy = {
    receiveEvent: receiveEvent
  };
})();
...
// WebApp
(function () {
  var Utils = window.MiniAppX.Utils;
  var WebView = window.MiniAppX.WebView;
  ...
  function setBackgroundColor(color) {
    ...
    if (color == 'bg_color' || color == 'secondary_bg_color') {
      ...
      if (!bg_color) {
        console.error('[MiniAppX.WebApp] Background color format is invalid', color);
    ...
    function triggerFeedback(params) {
      ...
      if (params.type == 'impact') {
        if (params.impact_style != 'light' &&
          ...
          console.error('[MiniAppX.WebApp] Haptic impact style is invalid', params.impact_style);
        ...
        if (params.notification_type != 'error' &&
          ...
          console.error('[MiniAppX.WebApp] Haptic notification type is invalid', params.notification_type);
          throw Error('WebAppHapticNotificationTypeInvalid');
        }
      } else if (params.type == 'selection_change') {
        // no params needed
      } else {
        console.error('[MiniAppX.WebApp] Haptic feedback type is invalid', params.type);
  ...
  var BiometricManager = (function() {
    ...
    function checkInit() {
      if (!isInited) {
        console.error('[MiniAppX.WebApp] BiometricManager should be inited before using.');
    ...
    biometricManager.requestAccess = function(params, callback) {
      ...
      checkInit();
      if (!isBiometricAvailable) {
        console.error('[MiniAppX.WebApp] Biometrics is not available on this device.');
        throw Error('WebAppBiometricManagerBiometricsNotAvailable');
      }
      if (accessRequestState) {
        console.error('[MiniAppX.WebApp] Access is already requested');
      ...
      if (typeof params.reason !== 'undefined') {
        ...
        if (reason.length > 128) {
          console.error('[MiniAppX.WebApp] Biometric reason is too long', reason);
    ...
    biometricManager.authenticate = function(params, callback) {
      ...
      if (!isAccessGranted) {
        console.error('[MiniAppX.WebApp] Biometric access was not granted by the user.');
        throw Error('WebAppBiometricManagerBiometricAccessNotGranted');
      }
      if (authRequestState) {
        console.error('[MiniAppX.WebApp] Authentication request is already in progress.');
    ...
    biometricManager.updateBiometricToken = function(token, callback) {
      ...
      if (token.length > 1024) {
        console.error('[MiniAppX.WebApp] Token is too long', token);
      ...
      if (tokenRequestState) {
        console.error('[MiniAppX.WebApp] Token request is already in progress.');
    ...
    biometricManager.openSettings = function() {
      ...
      if (!isAccessRequested) {
        console.error('[MiniAppX.WebApp] Biometric access was not requested yet.');
        throw Error('WebAppBiometricManagerBiometricsAccessNotRequested');
      }
      if (isAccessGranted) {
        console.warn('[MiniAppX.WebApp] Biometric access was granted by the user, no need to go to settings.');
        return biometricManager;
      }
  ...
  WebApp.sendData = function (data) {
    if (!data || !data.length) {
      console.error('[MiniAppX.WebApp] Data is required', data);
      throw Error('WebAppDataInvalid');
    }
    if (byteLength(data) > 4096) {
      console.error('[MiniAppX.WebApp] Data is too long', data);
      throw Error('WebAppDataInvalid');
    }
  ...
  WebApp.switchInlineQuery = function (query, choose_chat_types) {
    ...
    query = query || '';
    if (query.length > 256) {
      console.error('[MiniAppX.WebApp] Inline query is too long', query);
      throw Error('WebAppInlineQueryInvalid');
    }
    var chat_types = [];
    if (choose_chat_types) {
      if (!Array.isArray(choose_chat_types)) {
        console.error('[MiniAppX.WebApp] Choose chat types should be an array', choose_chat_types);
      ...
      for (var i = 0; i < choose_chat_types.length; i++) {
        ...
        if (!good_types[chat_type]) {
          console.error('[MiniAppX.WebApp] Choose chat type is invalid', chat_type);
  ...
  WebApp.openInvoice = function (url, callback) {
    ...
    if (webAppInvoices[slug]) {
      console.error('[MiniAppX.WebApp] Invoice is already opened');
  ...
  WebApp.showPopup = function (params, callback) {
    ...
    if (typeof params.title !== 'undefined') {
      ...
      if (title.length > 64) {
        console.error('[MiniAppX.WebApp] Popup title is too long', title);
    ...
    if (!message.length) {
      console.error('[MiniAppX.WebApp] Popup message is required', params.message);
      throw Error('WebAppPopupParamInvalid');
    }
    if (message.length > 256) {
      console.error('[MiniAppX.WebApp] Popup message is too long', message);
      throw Error('WebAppPopupParamInvalid');
    }
    popup_params.message = message;
    if (typeof params.buttons !== 'undefined') {
      if (!Array.isArray(params.buttons)) {
        console.error('[MiniAppX.WebApp] Popup buttons should be an array', params.buttons);
        throw Error('WebAppPopupParamInvalid');
      }
      for (var i = 0; i < params.buttons.length; i++) {
        ...
        if (typeof button.id !== 'undefined') {
          ...
          if (id.length > 64) {
            console.error('[MiniAppX.WebApp] Popup button id is too long', id);
            throw Error('WebAppPopupParamInvalid');
          }
        }
        btn.id = id;
        ...
        if (button_type == 'ok' ||
          ...
          if (!text.length) {
            console.error('[MiniAppX.WebApp] Popup button text is required for type ' + button_type, button.text);
            throw Error('WebAppPopupParamInvalid');
          }
          if (text.length > 64) {
            console.error('[MiniAppX.WebApp] Popup button text is too long', text);
            throw Error('WebAppPopupParamInvalid');
          }
          btn.text = text;
        } else {
          console.error('[MiniAppX.WebApp] Popup button type is invalid', button_type);
          throw Error('WebAppPopupParamInvalid');
        }
        buttons.push(btn);
      }
    ...
    if (buttons.length < 1) {
      console.error('[MiniAppX.WebApp] Popup should have at least one button');
      throw Error('WebAppPopupParamInvalid');
    }
    if (buttons.length > 3) {
      console.error('[MiniAppX.WebApp] Popup should not have more than 3 buttons');
      throw Error('WebAppPopupParamInvalid');
    }
  ...
  WebApp.showScanQrPopup = function (params, callback) {
    ...
    if (typeof params.text !== 'undefined') {
      ...
      if (text.length > 64) {
        console.error('[MiniAppX.WebApp] Scan QR popup text is too long', text);
  ...
  WebApp.requestWriteAccess = function (callback) {
    ...
    if (WebAppWriteAccessRequested) {
      console.error('[MiniAppX.WebApp] Write access is already requested');
  ...
  WebApp.requestContact = function (callback) {
    ...
    if (WebAppContactRequested) {
      console.error('[MiniAppX.WebApp] Contact is already requested');
